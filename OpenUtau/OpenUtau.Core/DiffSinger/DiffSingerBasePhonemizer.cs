...
namespace OpenUtau.Core.DiffSinger
{
    public abstract class DiffSingerBasePhonemizer : MachineLearningPhonemizer
    {
        ...
        public override void SetSinger(USinger singer) {
            Log.Information($"DiffSingerBasePhonemizer.SetSinger: singer={singer?.Name}, location={singer?.Location}, _singerLoaded={_singerLoaded}");
            if (_singerLoaded && singer == this.singer) return;
            try {
                _singerLoaded = _executeSetSinger(singer);
                Log.Information($"DiffSingerBasePhonemizer.SetSinger: _singerLoaded={_singerLoaded}");
            } catch (Exception ex) {
                Log.Error(ex, "DiffSingerBasePhonemizer.SetSinger failed");
                _singerLoaded = false;
                throw;
            }
        }
